        ...
        <div style="padding: 0px;">
            ...
            <div class="big"  >
                <script type="text/javascript">
                    function addFloor(){
                        document.Fname.action = "${pageContext.request.contextPath }/addFloor.jsp";
                        document.Fname.submit();
                    }
                    function queryFloors(){
                        document.Fname.action = "${pageContext.request.contextPath }/queryFloors.do";
                        document.Fname.submit();
                    }
                    function delFloors() {
                        document.Fname.action = "${pageContext.request.contextPath }/deleteFloors.do";
                        document.Fname.submit();
                    }
                </script>
                <form action="/selectByFName.do" method="get" class="Form">
                    <input type="text" name="floor_name" placeholder="楼栋编号">
                    <input type="submit" value="查询">
                </form>
                <div class="layui-btn-group demoTable">
                    <button class="layui-btn"  onclick="addFloor()">添加</button>
                    <button class="layui-btn"  onclick="queryFloors()">批量查看</button>
                    <button class="layui-btn"  onclick="delFloors()">批量删除</button>
                </div>
                <form action="" method="get" name="Fname">
                <table class="layui-table" lay-data="{width: 825, height:800, page:true}">
                    <thead>
                    <tr>
                        <th lay-data="{field:'src', width:117}">批量操作</th>
                        <th lay-data="{field:'floor_name', width:120}">楼栋编号</th>
                        <th lay-data="{field:'unit', width:110}">单元数量</th>
                        <th lay-data="{field:'level', width:110, sort: true}">层数</th>
                        <th lay-data="{field:'room_amount', width:180}">房间数/单元</th>
                        ...
                        <c:forEach items="${list}" var="floor">
                            <tr>
                                <td><input type="checkbox" value="${floor.id}" name="chick"></td>
                                <td>${floor.floor_name}</td>
                                <td>${floor.unit}</td>
                                <td>${floor.level}</td>
                                <td>${floor.room_amount}</td>
                                <td><a href="/updateFloor.do?id=${floor.id}">编辑</a>
                                    <a href="/delFloor.do?id=${floor.id}">删除</a></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                </table>
