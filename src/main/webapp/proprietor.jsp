        ...
        <div style="padding: 0px;">
            ...
            <div class="big"  >
                <script type="text/javascript">
                    function addUser(){
                        document.Fname.action = "${pageContext.request.contextPath }/addUser.jsp";
                        document.Fname.submit();
                    }
                    function queryUsers(){
                        document.Fname.action = "${pageContext.request.contextPath }/queryUsers.do";
                        document.Fname.submit();
                    }
                    function delUsers() {
                        document.Fname.action = "${pageContext.request.contextPath }/delUsers.do";
                        document.Fname.submit();
                    }
                    function queryExportUser() {
                            var url="${pageContext.request.contextPath }/queryExportUser.do"
                            window.location.href = url;
                    }
                </script>
                <div class="layui-btn-group demoTable">
                    <button class="layui-btn" data-type="isAll" onclick="addUser()">添加</button>
                    <button class="layui-btn" data-type="getCheckData" onclick="queryUsers()">批量查看</button>
                    <button class="layui-btn" data-type="getCheckLength" onclick="delUsers()">批量删除</button>
                    <button class="layui-btn" data-type="isAll" onclick="queryExportUser()">导出表格</button>
                </div>
                <form action="" method="get" name="Fname">
                <table class="layui-table" lay-data="{width: 1338, height:800, page:true}">
                    <thead>
                    <tr>
                        ...
                        <th lay-data="{field:'date', width:160, sort: true}">修改日期</th>
                        ...
                        <c:forEach items="${list}" var="user">
                            <tr>
                                ...
                                <td><a href="/upUser.do?id=${user.id}">编辑</a>
                                    <a href="/delUser.do?id=${user.id}">删除</a></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                </table>
